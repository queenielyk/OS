...
public class SleepingBarber extends Thread {
	 ...
	 * asleep so we call the constructor with parameter 0 thus creating semaphores
	 * with zero initial permits. Semaphore(1) constructs a binary semaphore, as
	 * desired.
	 */
	...
	/*
	 * we create the integer numberOfFreeSeats so that the customers can either sit
	 * on a free seat or leave the barbershop if there are no seats available
	 */

	public static int numberOfFreeSeats;
	public final int nBarber;

	public SleepingBarber(int m, int n) {
		nBarber = new Integer(m);
		numberOfFreeSeats = new Integer(n);
	...
	class Customer extends Thread {
		...
		/*
		 * we create the integer iD which is a unique ID number for every customer and a
		 * boolean notCut which is used in the Customer waiting loop
		 */
			...
			while (notCut) { // as long as the customer is not cut
				try {
					...
					if (numberOfFreeSeats > 0) { // if there are any free seats
						System.out.println("Customer " + this.iD + " just sat down in WR.");
	...
	class Barber extends Thread {
		...
		public Barber(int value) {
			ID = value;
		}

		public void run() {
			System.out.println("Barber "+ID+" ready for the Barber shop");
	...
	public static void main(String args[]) {
		...
		if (WR < iBarber)
			System.out.println("Incorrect input, Waiting Room must be more than Barbers.");
		else {
			SleepingBarber barberShop = new SleepingBarber(iBarber, WR); // Creates a new barbershop
	...
	public void run() {
		...
		for (int n = 1; n <= nBarber; n++) {
			Barber giovanni = new Barber(n); // Giovanni is the best barber ever
